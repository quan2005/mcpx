...
export default function Resources() {
  ...
  const [resourceContent, setResourceContent] = useState<unknown[] | null>(null)
  ...
  const filteredResources = resources.filter(
    (resource) =>
      ...
      resource.uri.toLowerCase().includes(filter.toLowerCase())
  )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          {Object.entries(groupedResources).length === 0 ? (
            ...
            Object.entries(groupedResources).map(([server, serverResources]) => (
                ...
                <div className="space-y-2">
                  {serverResources.map((resource) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-slate-400 truncate">{resource.uri}</div>
                      {resource.mime_type && (
                        <div className="text-xs text-slate-500 mt-1">{resource.mime_type}</div>
                      )}
                    </button>
                  ))}
                </div>
          ...
          {selectedResource ? (
            <div>
              ...
              <div className="mb-4">
                ...
                <p className="text-white font-mono text-sm break-all">{selectedResource.uri}</p>
              </div>
              ...
              <div className="mt-6">
                ...
                {loading ? (
                  ...
                  <div className="text-slate-500">Click a resource to view its content</div>
                )}
              </div>
            </div>
          ) : (
