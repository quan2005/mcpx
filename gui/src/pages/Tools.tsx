...
export default function Tools() {
  ...
  const handleToolClick = async (server: string, tool: string) => {
    ...
    try {
      ...
      showToast({
        type: "error",
        message: `Failed to load tool details: ${err}`,
      });
    }
  };
  ...
  const filteredTools = tools.filter(
    (tool) =>
      tool.name.toLowerCase().includes(debouncedFilter.toLowerCase()) ||
      tool.description.toLowerCase().includes(debouncedFilter.toLowerCase()),
  );
      ...
      {selectedTool && (
        <ToolDetailModal
          ...
          enabled={
            ...
            tools.find(
              (t) =>
                t.server === selectedTool.server &&
                t.name === selectedTool.tool,
            )?.enabled ??
            true
          }
          onClose={() => {
